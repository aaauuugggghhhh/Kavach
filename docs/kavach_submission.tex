...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Dynamic Track Node
    \node (s2b) [card] at (2.8, -2.2) {\textbf{Stage 2B: Dynamic Track}\\ADB Detonator + Frida + eBPF\\Native Emulation Sandbox};

    % Merge and Output Nodes
    \node (s6) [card] at (6.8, -3.2) {\textbf{Stage 6: Telemetry Merge}\\Contradiction Resolution\\\& BCNF Serialization};
    \node (s7) [card, fill=white] at (6.8, -4.9) {\textbf{Stage 7: React SOC}\\Dashboard View, PDF Export\\\& LLaMA-3 Groq Report};
...
\subsubsection*{Stage 2B: Dynamic Track: Native "Holy Trinity" Sandbox (ADB Detonator + Frida + eBPF)}

Simultaneously, the APK is routed to Kavach.ai's native dynamic analysis pipeline. By combining direct \textbf{ADB Device Orchestration}, \textbf{In-Memory Frida Bypass Hooks}, and \textbf{Kernel-Level eBPF Telemetry Tracing}, the system observes malware behavior at both user-land and kernel-land in under 20 seconds without third-party sandbox latency or privacy leaks.

Conceptually, the sandbox functions like an airport security checkpoint:
\begin{itemize}
  \item \textbf{The Airport Interrogator (\texttt{detonate.py}):} Manages device connectivity via ADB, installs the target binary, spawns Frida sessions, and broadcasts system intents (\texttt{BOOT\_COMPLETED}, \texttt{BATTERY\_LOW}) to detonate sleeping banking trojans.
  \item \textbf{The Undercover Spy (\texttt{frida\_bypass.js}):} Injected into JVM memory to spoof root checks (\texttt{/system/bin/su}, \texttt{ro.build.tags} to \texttt{release-keys}), bypass SSL pinning (\texttt{SSLContext.init}, \texttt{X509TrustManager}), and extract file I/O and network socket telemetry.
  \item \textbf{The Invisible Kernel Camera (\texttt{ebpf\_trace.py}):} Operates beneath user-land at the Linux kernel layer, tracing system calls (\texttt{sys\_clone}, \texttt{sys\_execve}, \texttt{sys\_socket}, \texttt{sys\_connect}, \texttt{sys\_write}, \texttt{sys\_openat}) and providing high-fidelity fallback simulation telemetry when physical devices are offline.
\end{itemize}

\begin{figure}[H]
  \centering
  \begin{mdframed}[backgroundcolor=black!2,linecolor=black!40,innerleftmargin=10pt,innerrightmargin=10pt,topline=true,bottomline=true]
  \small
  \begin{verbatim}
[APK Upload]
   │
   ▼
[detonate.py (ADB Detonator)] ──► Checks ADB Connection
   │                             ├── [Device Attached] ──► Installs APK & Triggers Intents
   │                             └── [No Device] ────────► Activates EBPFTracker Simulator
   ├──► [frida_bypass.js] ─────► Bypasses Root & SSL Pinning; Logs File/Socket Access
   └──► [ebpf_trace.py]   ─────► Traces sys_connect, sys_openat, sys_execve
         │
         ▼
  [telemetry.json] ───────► [arq_worker.py] ───────► [merge_telemetry()]
  (Structured Logs)         (Dynamic Result Mapping)  (Final Risk Score & Overrides)
  \end{verbatim}
  \end{mdframed}
  \caption{Dynamic Analysis Data Routing Flow}
  \label{fig:dynamic_flow}
\end{figure}

The extracted telemetry details are summarized below:
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Component Script} & \textbf{Target Domain} & \textbf{Extracted Telemetry \& Bypass Operations} \\ \hline
\texttt{detonate.py} & ADB & Device serials, intent broadcasts (\texttt{BOOT\_COMPLETED}), installation metrics. \\ \hline
\texttt{frida\_bypass.js} & User-Land JVM & Root check overrides (\texttt{su}, \texttt{busybox}), SSL pinning bypass, socket IPs. \\ \hline
\texttt{ebpf\_trace.py} & Kernel-Land & Syscalls (\texttt{sys\_connect}, \texttt{sys\_openat}), shared prefs, \texttt{telemetry.json} serialization. \\ \hline
\end{tabular}
\caption{Dynamic Sandbox Component Telemetry Matrix}
...
To avoid the monolithic bottleneck where heavy static calculations and dynamic sandboxing block the user interface, Kavach.ai separates execution into a decoupled microservices architecture utilizing a React frontend, a FastAPI asynchronous orchestrator, and a normalized PostgreSQL state manager.

Under the FastAPI asynchronous model ("Restaurant Ticket" paradigm), the API accepts incoming APK binaries, vaults them in the local PostgreSQL database (stashed as secure BYTEA payloads or local secure offline storage), records an execution transaction in PostgreSQL, and immediately hands a unique \texttt{Job ID} ticket back to the React client. The heavy analysis is then routed to background Celery or ARQ worker processes, freeing the API to process concurrent requests.

The React dashboard implements client-side polling, sending light requests to the FastAPI backend every two seconds using the \texttt{Job ID} ticket:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Asynchronous Dynamic Track (15--20 Seconds):} Runs in parallel. The orchestrator triggers native ADB detonation, injects root-bypass and SSL-pinning hooks via Frida (\texttt{frida\_bypass.js}), and inserts the stealthy eBPF kernel probe. Telemetry is gathered for a 15-second observation window and returned immediately.
  \item \textbf{Dynamic Dashboard Update:} Once the dynamic track finishes, the React dashboard updates the score and merges the user-land memory dumps and kernel-space logs into the active case file without requiring any manual analyst refresh.
\end{itemize}

\subsubsection*{Stage 7: Operational Deployment: React SOC Dashboard}
...
\midrule
Frontend \& Orchestration & \textbf{React} (SOC Analyst Dashboard), \textbf{FastAPI} microservices, \textbf{SQLModel} ORM session validation, and a \textbf{Redis}-backed asynchronous task runner. \\
\addlinespace
...
\addlinespace
Dynamic Analysis & \textbf{ADB Detonator, Frida Hooks, and eBPF} (local "Holy Trinity" dynamic sandbox orchestration for user-land bypasses and kernel-land telemetry collection). \\
\addlinespace
...
|   |-- docker-compose.yml      # WSL PostgreSQL & Redis containers natively
|   |-- Dockerfile.api          # FastAPI API service builder
|   `-- Dockerfile.React    # React dashboard UI builder
